...
interface QuotationForm {
  ...
  terms: string; notes: string; validUntil: string;
...
export function CreateQuotationModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  const [form, setForm] = useState<QuotationForm>(emptyForm());
  const [error, setError] = useState('');

  const { data: ordersData } = useQuery({
    queryKey: ['orders-for-quotation'],
  ...
  const set = (key: keyof QuotationForm, val: string) => setForm((f) => ({ ...f, [key]: val }));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-base font-bold text-slate-900">Create Quotation</h2>
            <p className="text-xs text-slate-400 mt-0.5">Quotation number auto-generated on save</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelCls}>Valid Until <span className="text-slate-400 font-normal">(optional)</span></label>
              <input type="date" value={form.validUntil} onChange={(e) => set('validUntil', e.target.value)} className={inputCls} />
            </div>
          </div>

          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Quote To</p>
          ...
          <div>
            ...
            <textarea rows={3} value={form.notes} onChange={(e) => set('notes', e.target.value)}
              placeholder="Additional notes or message to client…"
              className={`${inputCls} resize-none`} />
          </div>
        </div>

        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-3">
          <span className="text-xs text-slate-400">Quotation number auto-generated on save</span>
